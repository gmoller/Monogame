...
namespace WarlordsRevenge.FarseerSamples
{
    ...
    {
        private const float NUM_ENTRIES = 15;
        ...
        public override void LoadContent()
        {
            ...
            _menuBorderTop = (viewport.Height - _menuBorderMargin * (NUM_ENTRIES - 1)) / 2f;
            _menuBorderBottom = (viewport.Height + _menuBorderMargin * (NUM_ENTRIES - 1)) / 2f;

            _menuOffset = 0f;
            _maxOffset = Math.Max(0f, (_menuEntries.Count - NUM_ENTRIES) * _menuBorderMargin);
        ...
        private int GetMenuEntryAt(Vector2 position)
        {
            ...
            foreach (MenuEntry entry in _menuEntries)
            {
                ...
                var rect = new Rectangle((int)(entry.Position.X - width / 2f), (int)(entry.Position.Y - height / 2f), (int)width, (int)height);
